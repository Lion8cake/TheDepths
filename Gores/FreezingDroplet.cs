...
namespace TheDepths.Gores
{
    public class FreezingDroplet : ModGore
    {
        ...
        public override bool Update(Gore gore)
        {
            ...
            if (gore.frame <= 4)
            {
                ...
                if (gore.frameCounter >= frameDuration)
                {
                    ...
                    if (gore.frame == 5)
                    {
                        int droplet = Gore.NewGore(new EntitySource_Misc(nameof(FreezingDroplet)), gore.position, gore.velocity, gore.type);
