...
namespace TheDepths.Worldgen.UndergroundHouse.CaveHouse
{
	public class HouseBuilder
	{
				...
				for (int i = 0; i < num; i++)
				{
					...
					case 1:
					{
						...
						for (int j = -1; j < 2; j++)
						{
							for (int k = 0; k < 3; k++)
							{
								Main.tile[j + num4, k + num5].TileFrameX += 54;
							}
						}
						break;
					}
		...
		private void PlaceSupportBeams()
		{
			...
			{
				if (item.Height > 1 && Main.tile[item.X, item.Y - 1].TileType != 19)
				{
					...
					Tile tile = Main.tile[item.X, item.Y + item.Height];
					tile.Slope = 0;
		...
		private void PlaceChests()
		{
			...
			foreach (Rectangle room in Rooms)
			{
				...
				for (int i = 0; i < 10; i++)
				{
					if (flag = Worldgen.TheDepthsWorldGen.AddBuriedChest(_random.Next(2, room.Width - 2) + room.X, num, 0, notNearOtherChests: false, style, trySlope: false, chestTileType))
					{
						break;
					}
				}
				...
				{
					if (flag = Worldgen.TheDepthsWorldGen.AddBuriedChest(j, num, 0, notNearOtherChests: false, style, trySlope: false, chestTileType))
					{
						break;
					}
				}
			...
			if (!flag)
			{
				foreach (Rectangle room2 in Rooms)
				{
					...
					for (int k = 0; k < 10; k++)
					{
						if (flag = Worldgen.TheDepthsWorldGen.AddBuriedChest(_random.Next(2, room2.Width - 2) + room2.X, num3, 0, notNearOtherChests: false, style2, trySlope: false, chestTileType2))
						{
							break;
						}
					}
					...
					{
						if (flag = Worldgen.TheDepthsWorldGen.AddBuriedChest(l, num3, 0, notNearOtherChests: false, style2, trySlope: false, chestTileType2))
						{
							break;
						}
					}
			...
			for (int m = 0; m < 1000; m++)
			{
				...
				if (flag = Worldgen.TheDepthsWorldGen.AddBuriedChest(i2, num5, 0, notNearOtherChests: false, style3, trySlope: false, chestTileType3))
				{
					break;
				}
			}
		...
		private void PlaceBiomeSpecificPriorityTool(HouseBuilderContext context)
		{
			...
			foreach (Rectangle room in Rooms)
			{
				...
				for (int i = 0; i < 10; i++)
				{
					...
					if (flag = Main.tile[num2, num].HasTile && Main.tile[num2, num].TileType == 506)
					{
						break;
					}
				}
			...
			if (!flag)
			{
				foreach (Rectangle room2 in Rooms)
				{
					...
					for (int k = 0; k < 10; k++)
					{
						...
						if (flag = Main.tile[num4, num3].HasTile && Main.tile[num4, num3].TileType == 506)
						{
							break;
						}
					}
		...
		private void PlaceBiomeSpecificTool(HouseBuilderContext context)
		{
			if (Type == HouseType.Jungle && context.SharpenerCount < _random.Next(2, 5))
			{
				...
				foreach (Rectangle room in Rooms)
				{
					...
					for (int i = 0; i < 10; i++)
					{
						...
						if (flag = Main.tile[num2, num].HasTile && Main.tile[num2, num].TileType == 377)
						{
							break;
						}
					}
			...
			foreach (Rectangle room2 in Rooms)
			{
				...
				for (int k = 0; k < 10; k++)
				{
					...
					if (flag2 = Main.tile[num4, num3].HasTile && Main.tile[num4, num3].TileType == 219)
					{
						break;
					}
				}
