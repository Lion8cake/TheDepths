	...
	public class LumpOfCoal : ModProjectile
	{
		...
		public override void AI()
		{
			...
			for (int num103 = 0; num103 < 2; num103++)
			{
				...
				int num101 = 4;
				int frostDust = Dust.NewDust(new Vector2(Projectile.position.X + (float)num101, Projectile.position.Y + (float)num101), Projectile.width - num101 * 2, Projectile.height - num101 * 2, DustID.Torch, 0f, 0f, 100, default(Color), 1.2f);
			...
			{
				int num102 = 4;
				int frostDustSmol = Dust.NewDust(new Vector2(Projectile.position.X + (float)num102, Projectile.position.Y + (float)num102), Projectile.width - num102 * 2, Projectile.height - num102 * 2, DustID.Torch, 0f, 0f, 100, default(Color), 0.6f);
