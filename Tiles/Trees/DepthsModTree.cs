...
{
    public abstract class DepthsModTree : ModTile
    {
		public virtual string TexturePath => "TheDepths/Tiles/Trees/PetrifiedTree";
		...
		public static void KillTile_GetTreeDrops(int i, int j, Tile tileCache, ref bool bonusWood, ref int dropItem, ref int secondaryItem)
		{
			...
			{
				dropItem = ModContent.ItemType<Items.Placeable.NightWood>();
			}
		...
		{
			return type == ModContent.TileType<ShaleBlock>() || type == ModContent.TileType<NightmareGrass>();
		}
		...
		{
			if (tileType == ModContent.TileType<PetrifiedTree>())
			{
				return PetrifiedTree.TreePetrified;
			}
			else if (tileType == ModContent.TileType<NightwoodTree>())
			{
				return NightwoodTree.TreeNightmare;
			}
			else if (tileType == ModContent.TileType<OnyxGemtree>())
			{
				return OnyxGemtree.GemTreeOnyx;
			}
			else
			{
				return null;
		...
		public class TreeTopRenderTargetHolder : ARenderTargetHolder
		{
			...
			public override void Prepare()
			{
				...
				if (Key.TextureIndex == ModContent.TileType<PetrifiedTree>())
				{
					asset = ModContent.Request<Texture2D>("TheDepths/Tiles/Trees/PetrifiedTree_Tops");
				}
				else if (Key.TextureIndex == ModContent.TileType<NightwoodTree>())
				{
					asset = ModContent.Request<Texture2D>("TheDepths/Tiles/Trees/NightwoodTree_Tops");
				}
				else
				{
					asset = ModContent.Request<Texture2D>("TheDepths/Tiles/Trees/OnyxGemtree_Tops");
				}
				if (asset == null)
				{
		...
		public class TreeBranchTargetHolder : ARenderTargetHolder
		{
			...
			public override void Prepare()
			{
				...
				if (Key.TextureIndex == ModContent.TileType<PetrifiedTree>())
				{
					asset = ModContent.Request<Texture2D>("TheDepths/Tiles/Trees/PetrifiedTree_Branches");
				}
				else if (Key.TextureIndex == ModContent.TileType<NightwoodTree>())
				{
					asset = ModContent.Request<Texture2D>("TheDepths/Tiles/Trees/NightwoodTree_Branches");
				}
				else
				{
					asset = ModContent.Request<Texture2D>("TheDepths/Tiles/Trees/OnyxGemtree_Branches");
				}
				if (asset == null)
				{
