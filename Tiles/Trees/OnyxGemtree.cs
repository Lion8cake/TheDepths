...
{
    public class OnyxGemtree : DepthsModTree
    {
		public override string TexturePath => "TheDepths/Tiles/Trees/OnyxGemtree";

		public static GrowTreeSettings GemTree_Onyx = new GrowTreeSettings
		{
			GroundTest = GemTreeGroundTest,
			WallTest = GemTreeWallTest,
			TreeHeightMax = 12,
			TreeHeightMin = 7,
			TreeTileType = (ushort)ModContent.TileType<OnyxGemtree>(),
			TreeTopPaddingNeeded = 4,
			SaplingTileType = (ushort)ModContent.TileType<OnyxGemtreeSapling>()
		};

		public static TreePaintingSettings GemTreeOnyx = new TreePaintingSettings
		{
		...
		{
			int dustType = ((!WorldGen.genRand.NextBool(10)) ? 1 : ModContent.DustType<GemOnyxDust>());
		...
		public override IEnumerable<Item> GetItemDrops(int i, int j)
		{
			...
			int dropItemStack = 1;
			SetGemTreeDrops(ModContent.ItemType<Onyx>(), ModContent.ItemType<OnyxGemcorn>(), tileCache, ref dropItem, ref secondaryItem);
			if (dropItem == 3)
			{
		...
		private static void EmitOnyxLeaves(int tilePosX, int tilePosY, int grassPosX, int grassPosY)
		{
			...
			{
				Gore.NewGoreDirect(new EntitySource_Misc(""), vector, Utils.RandomVector2(Main.rand, -num3, num3), ModContent.GoreType<OnyxGemtreeLeaf>(), 0.7f + Main.rand.NextFloat() * 0.6f).Frame.CurrentColumn = Main.tile[tilePosX, tilePosY].TileColor;
			}
		}
					...
					case 22:
						{
							...
							EmitOnyxLeaves(x, y, grassPosX, floorY3);
					...
					case 44:
						{
							...
							EmitOnyxLeaves(x, y, num21 + num2, floorY2);
					...
					case 66:
						{
							...
							EmitOnyxLeaves(x, y, num17 + num18, floorY);
