...
namespace TheDepths.Tiles
{
	public class QuicksilverSensor : ModTile
	{
		...
		public override void SetDrawPositions(int i, int j, ref int width, ref int offsetY, ref int height, ref short tileFrameX, ref short tileFrameY)
		{
			...
			if ((Main.tile[x, y].LiquidType == ModContent.LiquidType<Quicksilver>()) && Main.tile[x, y].TileFrameX == 0)
			{
				Wiring.TripWire(x, y, 1, 1);
			}
			if (!(Main.tile[x, y].LiquidType == ModContent.LiquidType<Quicksilver>()) && Main.tile[x, y].TileFrameX == 18)
			{
				Wiring.TripWire(x, y, 1, 1);
			}

			if (Main.tile[x, y].LiquidType == ModContent.LiquidType<Quicksilver>())
			{
