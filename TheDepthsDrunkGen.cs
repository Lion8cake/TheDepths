...
namespace TheDepths
{
    public class TheDepthsDrunkGen : ModSystem 
	{
		...
		public static void AddDepthHousesLeft()
		{
			...
			for (int i = 100; i < (Main.maxTilesX / 2) - 100; i++)
			{
				if (((!WorldGen.drunkWorldGen && !WorldGen.remixWorldGen) || i <= num || i >= Main.maxTilesX - num) && (WorldGen.drunkWorldGen || WorldGen.remixWorldGen || (i >= num && i <= Main.maxTilesX - num)))
				{
					...
					if (Main.tile[i, num2 + 1].HasTile)
					{
						...
						DepthFortLeft(i, num2, num3, wallType);
			...
			num10 = 420000.0 / (double)(Main.maxTilesX / 2);
			int num24 = 0;
			while ((double)num24 < num10)
			{
				int num25 = 0;
				int num26 = WorldGen.genRand.Next(num, (Main.maxTilesX / 2) - num);
				...
				while ((Main.tile[num26, num27].WallType != 13 && Main.tile[num26, num27].WallType != 14) || Main.tile[num26, num27].HasTile)
				{
					...
					if (WorldGen.drunkWorldGen || WorldGen.remixWorldGen)
					{
						...
						{
							num26 = WorldGen.genRand.Next((Main.maxTilesX / 2) - num, (Main.maxTilesX / 2) - 50);
						}
					}
					num25++;
					if (num25 > 100000)
			...
			while ((double)num35 < num10)
			{
				...
				do
				{
					...
					if (WorldGen.drunkWorldGen || WorldGen.remixWorldGen)
					{
						...
						{
							num37 = WorldGen.genRand.Next((Main.maxTilesX / 2) - num, (Main.maxTilesX / 2) - 50);
						}
					}
					num36++;
				}
		...
		public static void DepthFortLeft(int i, int j, ushort tileType = 75, ushort wallType = 14)
		{
			...
			for (int l = 0; l < 5; l++)
			{
				int num22 = array14[l];
				bool flag3 = false;
				if (num22 < 10 || num22 > (Main.maxTilesX / 2) - 10)
				{
					flag3 = true;
				}
				else
				...
				for (int num33 = 0; num33 < 10; num33++)
				{
					if (array10[num50, num33])
					{
						if (num33 > 0 && array10[num50, num33 - 1])
						{
							...
							for (int num29 = num32; num29 <= num31 && num29 >= 20 && num29 <= (Main.maxTilesX / 2) - 20; num29++)
							{
			...
			while (flag5)
			{
				...
				if (array7[num41])
				{
					...
					for (int num37 = num40; num37 <= num39 && num37 >= 10 && num37 <= (Main.maxTilesX / 2) - 10; num37++)
					{
					...
					if (flag5)
					{
						for (int num36 = num40; num36 <= num39 && num36 >= 10 && num36 <= (Main.maxTilesX / 2) - 10; num36++)
						{
		...
		{
			{
				...
				if (WorldGen.remixWorldGen && !WorldGen.drunkWorldGen)
				{
					...
					for (; num857 < Main.maxTilesY - 1 || num856 < num855; num856++)
					{
						...
						{
							if ((num856 <= Main.maxTilesX - 5 || num856 >= Main.maxTilesX + 5) && WorldGen.genRand.Next(4) == 0)
							{
								if (WorldGen.genRand.Next(3) == 0)
								{
									num857 += WorldGen.genRand.Next(-1, 2);
				...
				else if (!WorldGen.drunkWorldGen)
				{
					...
					for (int num868 = 25 + Main.maxTilesX / 2; num868 < Main.maxTilesX - 25; num868++)
					{
						if ((double)num868 < (double)Main.maxTilesX * 0.17 || (double)num868 > (double)Main.maxTilesX * 0.83)
						{
							for (int num869 = Main.maxTilesY - 200; num869 < Main.maxTilesY - 50; num869++)
		...
		public static void AddDepthHousesRight()
		{
			int num = (int)((double)Main.maxTilesX * 0.25);
			for (int i = 100 + Main.maxTilesX / 2; i < (Main.maxTilesX) - 100; i++)
			{
				if (((!WorldGen.drunkWorldGen && !WorldGen.remixWorldGen) || i <= num || i >= Main.maxTilesX - num) && (WorldGen.drunkWorldGen || WorldGen.remixWorldGen || (i >= num && i <= Main.maxTilesX - num)))
				{
					...
					if (Main.tile[i, num2 + 1].HasTile)
					{
						...
						DepthFortRight(i, num2, num3, wallType);
						i += WorldGen.genRand.Next(30, 130);
						if (WorldGen.genRand.Next(10) == 0)
						{
							i += WorldGen.genRand.Next(0, 200);
		...
		public static void DepthFortRight(int i, int j, ushort tileType = 75, ushort wallType = 14)
		{
			int[] array14 = new int[5];
			int[] array13 = new int[5];
			int[] array12 = new int[10];
			...
			while (flag5)
			{
				...
				if (array7[num41])
				{
					...
					for (int num37 = num40 + Main.maxTilesX / 2; num37 <= num39 && num37 >= 10 && num37 <= (Main.maxTilesX) - 10; num37++)
					{
						tile = Main.tile[num37, array12[num43] - 1];
						if (!tile.HasTile)
						{
					...
					if (flag5)
					{
						for (int num36 = num40 + Main.maxTilesX / 2; num36 <= num39 && num36 >= 10 && num36 <= (Main.maxTilesX) - 10; num36++)
						{
							tile = Main.tile[num36, array12[num43]];
							tile.HasTile = false;
							WorldGen.PlaceTile(num36, array12[num43], TileType<QuartzPlatform>(), true, true, -1);
